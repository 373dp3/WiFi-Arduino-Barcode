...
namespace WiFiBarcodeConfigSetter
{
    public partial class Form1 : Form
    {
        ...
        Regex ptnServer1 = new Regex("^http:\\/\\/[.]*", RegexOptions.IgnoreCase);
        Regex ptnServer2 = new Regex("[.]*\\/$", RegexOptions.IgnoreCase);
        Regex ptnNotZenkaku = new Regex("[^\\x01-\\x7E]");

        private void buttonWrite_Click(object sender, EventArgs e)
        {
            //アドレス形式に関する警告
            if (ptnServer1.IsMatch(textBoxServerIp.Text) == false)
            {
                MessageBox.Show("Server欄は必ずhttp://で開始してください");
                return;
            }
            if (ptnServer2.IsMatch(textBoxServerIp.Text) == false)
            {
                MessageBox.Show("Server欄の末尾は必ず / で終えてください");
                return;
            }
            if (ptnNotZenkaku.IsMatch(textBoxServerIp.Text)
                || ptnNotZenkaku.IsMatch(textBoxServerIp.Text)
                || ptnNotZenkaku.IsMatch(textBoxSSID.Text)
                || ptnNotZenkaku.IsMatch(textBoxPASSWD.Text)
                || ptnNotZenkaku.IsMatch(textBoxIP.Text)
                || ptnNotZenkaku.IsMatch(textBoxSubnetMask.Text)
                || ptnNotZenkaku.IsMatch(textBoxGateway.Text)
                || ptnNotZenkaku.IsMatch(textBoxDNS.Text)
                || ptnNotZenkaku.IsMatch(textBoxSheetName.Text)
                )
            {
                MessageBox.Show("全角文字はお使いいただけません。");
                return;
            }
