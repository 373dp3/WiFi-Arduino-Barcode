...
public class ScreenMgr {
	...
	public ScreenMgr(String json, String dbPath, final ServerConfig cfg) throws ClassNotFoundException, SQLException{
		...
		ShutDownThread th = new ShutDownThread(cfg);
	...
	class ShutDownThread extends Thread{
		Logger l = LogManager.getLogger(ShutDownThread.class);
		private ServerConfig cfg;
		public ShutDownThread(ServerConfig cfg){
			this.cfg = cfg;
		}
		public void run() {
			final SimpleDateFormat sdfHHMM = new SimpleDateFormat("yyyyMMdd_HHmm");
			File path = new File(cfg.upload.xlsxBackupDir);
			if(path.exists()==false){
				path.mkdirs();
			}
			String fileName = path.getAbsolutePath() + File.separator+ sdfHHMM.format(new Date()) + ".xlsx";
			l.debug(fileName);
			try{
